...
The CLI caches multiple accounts and tracks an **active** account that all other commands run as.

```bash
npx power-apps login                              # sign in (adds an account)
npx power-apps login --account user@contoso.com   # pre-fill the sign-in prompt
npx power-apps auth-status                        # show cached accounts (active is marked *)
npx power-apps auth-switch                        # choose the active account (prompts if many)
npx power-apps auth-switch --account user@contoso.com  # set the active account non-interactively
npx power-apps logout                             # clear all cached accounts
```

| Command       | Description                                                                | Options             |
| ------------- | -------------------------------------------------------------------------- | ------------------- |
| `login`       | Sign in to Microsoft Entra ID and add the account to the local cache.      | `--account <email>` |
| `auth-status` | Show cached accounts; the active account (used by other commands) is `*`.  |                     |
| `auth-switch` | Choose which cached account other commands run as.                         | `--account <email>` |
| `logout`      | Clear all cached credentials (every account + the active-account pointer). |                     |

All four honor `--json` and print the result as pretty-printed JSON (same convention as the list commands).

When multiple accounts are cached and none is active, commands that need a token fail with guidance
to run `auth-switch`. `--account` accepts a username (email) or an MSAL home account ID. In
non-interactive mode (`--non-interactive`, CI, or piped output) the CLI never opens a browser — sign
in first with `login`, and pass `--account` to `auth-switch` rather than relying on the picker.
