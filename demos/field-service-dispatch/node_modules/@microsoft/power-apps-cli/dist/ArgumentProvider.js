...
import { createDefaultStringOption, getAuthResourceUrl } from './CliUtils.js';
...
// Flat verbs that manage authentication state rather than operate against an environment. They
// skip the environment-id requirement in `_getRequiredVariables`.
...
export class ArgumentProvider {
    ...
    async _getRequiredVariables() {
        ...
        // If there is no config we prompt for them, except for telemetry and the auth verbs
        // (login/logout/auth-status/auth-switch), which manage their own state and don't operate
        // against a specific environment.
        const skipEnvironmentId = this._verb === 'telemetry' || (this._verb !== undefined && AUTH_VERBS.includes(this._verb));
