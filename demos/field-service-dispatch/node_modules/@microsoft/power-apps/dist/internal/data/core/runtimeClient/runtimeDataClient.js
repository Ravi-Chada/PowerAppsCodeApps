...
import { arrayBufferToBase64, convertArrayBufferToString, extractDataverseUrlParts, } from '../common/utils';
...
export class RuntimeDataClient {
    ...
    static createInstanceAsync(powerOperationExecutor, createRequestBody) {
        return Promise.resolve(new RuntimeDataClient(powerOperationExecutor, createRequestBody));
    }
    /**
    ...
    async _executeRequest(config, context) {
        ...
        // If the bridge provides a createRequestBody hook (e.g. native/Hermes runtimes that
        // cannot use `new Blob([ArrayBuffer])`), delegate body construction to it.
        // Otherwise fall back to the default Blob-wrapping logic used by the web player.
        const requestBody = config.body
            ? this._createRequestBody
                ? this._createRequestBody(config)
